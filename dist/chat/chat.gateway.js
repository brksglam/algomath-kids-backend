...
let ChatGateway = class ChatGateway {
    ...
    async onJoinCourse(client, data) {
        if (!data?.courseId)
            return;
        await client.join('course:' + data.courseId);
    }
    async onMessage(_client, data) {
        if (!data?.courseId || !data?.senderId || !data?.content)
            return;
        try {
            const message = await this.chatService.createMessage(data.courseId, data.senderId, data.content, data.recipientId);
...
], ChatGateway.prototype, "onJoinCourse", null);
__decorate([
