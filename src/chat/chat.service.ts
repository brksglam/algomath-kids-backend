...
export class ChatService {
  ...
  async createMessage(
    ...
    const isTeacher = course.teachers.some((t) =>
      t.equals(new Types.ObjectId(senderId)),
    );
    const isStudent = course.students.some((s) =>
      s.equals(new Types.ObjectId(senderId)),
    );
    ...
    // Enforce chat policy
    const policy =
      (course as unknown as { chatPolicy?: ChatPolicy }).chatPolicy ??
      ChatPolicy.StudentsWithTeacher;
    if (policy === ChatPolicy.TeacherToStudents && !isTeacher) {
      throw new NotFoundException(
        'Only teachers can send messages in this course',
      );
    }
